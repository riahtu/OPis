...
**/
   ...
   if ( !( $database = mysql_connect( "localhost","root", "apmsetup" ) ) )                      
	...
	$query = "select * from twitter_words";
    $query_result = mysql_query( $query, $database ); 
	
	$wordNumber = 0;
	while($row = mysql_fetch_row($query_result)){
		$word_array[$wordNumber][0] = $row[0];
		$word_array[$wordNumber][1] = $row[1];
		$wordNumber++;
	}
...
//-------------------------------------------------------------- Format the tweet text (links, hashtags, mentions)
	...
	function scoringWords($tweet, $word_num, $word_array){

		$positive_flag = 0;
		$negative_flag = 0;

		for($for_p=0; $for_p<$word_num; $for_p++)
		{
			// 한 트윗에 긍정, 부정 단어 둘 다 있으면?
			if(strpos($tweet, $word_array[$for_p][0]) !== false)
			{
				if($word_array[$for_p][1] == 0)
				{
					print("negative word");
					$negative_flag = -1;
				}
...
//-------------------------------------------------------------- Timeline HTML output
	...
	foreach ($twitter_data as $tweet) {
		...
		$score += scoringWords($formattedTweet, $wordNumber, $word_array);
...
<?php 
	...
	$discount_fee = ($score+$dawnScore)*50;
